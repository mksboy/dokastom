(function (_, $) {
  ...
  var customMask = {
    "mask": "8(###)###-##-##",  // Замените XXX на нужные вам плейсхолдеры
    "cc": "RU",               // Код страны (если применимо)
    "name_en": "Russia",  // Имя страны на английском
    "desc_en": "Custom description",  // Описание на английском
    "name_ru": "Россия",  // Имя страны на русском
    "desc_ru": "Пользовательское описание"  // Описание на русском
  };
  ...
  function validatePhone($input) {
    ...
    if (is_custom_format && $input.data('enableCustomMask')) {
      ...
      // Check if the phone number starts with '+7' or '8' for Russia
      var phoneNumber = $input.val().replace(/\D/g, ''); // Remove non-numeric characters
      mask_is_valid = /^(?:\+7|8)/.test(phoneNumber);

      if (!mask_is_valid) {
        // If it doesn't start with '+7' or '8', check against defined masks
        mask_list.forEach(function (mask) {
          mask_is_valid = mask_is_valid || _toRegExp(mask.mask).test($input.val());
        });
      }
